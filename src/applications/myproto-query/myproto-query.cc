...
const string MyprotoQuery::PRETURN = "pReturn";
const string MyprotoQuery::PERIODIC = "periodic";
const string MyprotoQuery::PROVQUERY = "provQuery";
const string MyprotoQuery::Q1_ECAPERIODIC = "q1_ecaperiodic";
const string MyprotoQuery::RECORDS = "records";
const string MyprotoQuery::TUPLE = "tuple";

NS_LOG_COMPONENT_DEFINE ("MyprotoQuery");
NS_OBJECT_ENSURE_REGISTERED (MyprotoQuery);

TypeId
MyprotoQuery::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::rapidnet::myprotoquery::MyprotoQuery")
    .SetParent<RapidNetApplicationBase> ()
    .AddConstructor<MyprotoQuery> ()
    ;
  return tid;
}

MyprotoQuery::MyprotoQuery()
{
...
void
MyprotoQuery::StartApplication (void)
{
  NS_LOG_FUNCTION_NOARGS ();

  RapidNetApplicationBase::StartApplication ();
  m_event_q1_ecaperiodic=
    Simulator::Schedule (Seconds (0), &MyprotoQuery::Q1_ecaperiodic, this);
  m_count_q1_ecaperiodic = 0;
  RAPIDNET_LOG_INFO("MyprotoQuery Application Started");
}

void
MyprotoQuery::StopApplication ()
{
  NS_LOG_FUNCTION_NOARGS ();

  RapidNetApplicationBase::StopApplication ();
  Simulator::Cancel(m_event_q1_ecaperiodic);
  RAPIDNET_LOG_INFO("MyprotoQuery Application Stopped");
}

void
MyprotoQuery::InitDatabase ()
{
...
void
MyprotoQuery::DemuxRecv (Ptr<Tuple> tuple)
{
  ...
  if (++m_count_q1_ecaperiodic < 2)
    {
      m_event_q1_ecaperiodic = Simulator::Schedule (Seconds(4),
        &MyprotoQuery::Q1_ecaperiodic, this);
    }
}

void
MyprotoQuery::Q1_eca (Ptr<Tuple> q1_ecaperiodic)
{
...
void
MyprotoQuery::Q2_eca (Ptr<Tuple> pReturn)
{
